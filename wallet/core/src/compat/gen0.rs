//!
//! v0 (Calico Desktop style) account data decryption
//!

use crate::error::Error;
use crate::imports::*;
use calico_bip32::{ExtendedPrivateKey, Language, Mnemonic, Prefix, SecretKey};
...
// ---
// {"type":"calico-wallet","encryption":"default","version":1,"generator":"pwa","wallet":{"mnemonic":"hex"}}
...
fn legacy_v0_keydata_location() -> Result<(PathBuf, Options)> {
    let filename = if runtime::is_windows() {
        ...
        fs::resolve_path(&format!("{appdata}/Calico/calico.kpk"))?
    } else if runtime::is_macos() {
        fs::resolve_path("~/Library/Application Support/Calico/calico.kpk")?
    } else {
        fs::resolve_path("~/.calico/calico.kpk")?
    };

    let options = workflow_store::fs::Options::with_local_storage_key("calico-wallet");

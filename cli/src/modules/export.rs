use crate::imports::*;
use calico_wallet_core::account::{multisig::MultiSig, Account, BIP32_ACCOUNT_KIND, MULTISIG_ACCOUNT_KIND};
...
async fn export_multisig_account(ctx: Arc<CalicoCli>, account: Arc<MultiSig>) -> Result<()> {
    match &account.prv_key_data_ids() {
        ...
        Some(prv_key_data_ids) => {
            ...
            for (id, prv_key_data_id) in prv_key_data_ids.iter().enumerate() {
                ...
                let xpub_key: calico_bip32::ExtendedPublicKey<calico_bip32::secp256k1::PublicKey> =
...
async fn export_single_key_account(ctx: Arc<CalicoCli>, account: Arc<dyn Account>) -> Result<()> {

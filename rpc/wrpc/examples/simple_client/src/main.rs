// Example of simple client to connect with Calico node using wRPC connection and collect some node and network basic data

use calico_rpc_core::{api::rpc::RpcApi, GetBlockDagInfoResponse, GetServerInfoResponse};
use calico_wrpc_client::{
    client::{ConnectOptions, ConnectStrategy},
    prelude::NetworkId,
    prelude::NetworkType,
    result::Result,
    CalicoRpcClient, Resolver, WrpcEncoding,
};
...
async fn main() -> ExitCode {
    match check_node_status().await {
        Ok(_) => {
            println!("Well done! You successfully completed your first client connection to Calico node!");
            ExitCode::SUCCESS
        }
        Err(error) => {
...
async fn check_node_status() -> Result<()> {
    ...
    // Verify your Calico node is runnning with --rpclisten-borsh=0.0.0.0:17110 parameter
    ...
    // Define the network your Calico node is connected to
    ...
    let client = CalicoRpcClient::new(encoding, url, resolver, selected_network, subscription_context)?;
    ...
    // Connect to selected Calico node
    client.connect(Some(options)).await?;

    // Retrieve and show Calico node information
    ...
    // Retrieve and show Calico network information
    ...
    // Disconnect client from Calico node

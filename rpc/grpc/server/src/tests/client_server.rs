...
use calico_notify::scope::{NewBlockTemplateScope, Scope};
use calico_rpc_core::api::rpc::RpcApi;
use calico_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    calico_core::log::try_init_logger("info, calico_grpc_core=trace, calico_grpc_server=trace, calico_grpc_client=trace");

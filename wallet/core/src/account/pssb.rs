...
//! (Partial Signed Calico Transaction Bundles).
//!

pub use crate::error::Error;
use crate::imports::*;
use crate::tx::PaymentOutputs;
use calico_bip32::{DerivationPath, KeyFingerprint, PrivateKey};
use calico_consensus_client::UtxoEntry as ClientUTXO;
use calico_consensus_core::hashing::sighash::{calc_schnorr_signature_hash, SigHashReusedValuesUnsync};
use calico_consensus_core::tx::VerifiableTransaction;
use calico_consensus_core::tx::{TransactionInput, UtxoEntry};
use calico_txscript::extract_script_pub_key_address;
use calico_txscript::opcodes::codes::OpData65;
use calico_txscript::script_builder::ScriptBuilder;
use calico_wallet_core::tx::{Generator, GeneratorSettings, PaymentDestination, PendingTransaction};
pub use calico_wallet_psst::bundle::Bundle;
use calico_wallet_psst::prelude::KeySource;
use calico_wallet_psst::prelude::{Finalizer, Inner, SignInputOk, Signature, Signer};
pub use calico_wallet_psst::psst::{Creator, PSST};
use futures::stream;
...
pub fn psst_to_pending_transaction(
    ...
    let output: Vec<calico_consensus_core::tx::TransactionOutput> = signed_tx.outputs.clone();
    ...
    let pending_tx = PendingTransaction::try_new(
        ...
        0,
        0,
        calico_wallet_core::tx::DataKind::Final,
    )?;

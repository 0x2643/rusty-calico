...
use calico_consensus_core::hashing::sighash::{SigHashReusedValues, SigHashReusedValuesSync};
use calico_consensus_core::{
    hashing::sighash::SigHashReusedValuesUnsync,
    mass::Kip9Version,
    tx::{TransactionInput, VerifiableTransaction},
};
use calico_core::warn;
use calico_txscript::caches::Cache;
use calico_txscript::{get_sig_op_count, SigCacheKey, TxScriptEngine};
use calico_txscript_errors::TxScriptError;
    ...
    fn check_sequence_lock(tx: &impl VerifiableTransaction, pov_daa_score: u64) -> TxResult<()> {
        ...
        if tx.populated_inputs().filter(|(input, _)| input.sequence & SEQUENCE_LOCK_TIME_DISABLED != SEQUENCE_LOCK_TIME_DISABLED).any(
            |(input, entry)| {
                ...
                //
                // Note: in the calicod codebase there's a use in i64 in order to use the -1 value
...
mod tests {
    ...
    use calico_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use calico_consensus_core::sign::sign;
    use calico_consensus_core::subnets::SubnetworkId;
    use calico_consensus_core::tx::{MutableTransaction, PopulatedTransaction, ScriptVec, TransactionId, UtxoEntry};
    use calico_consensus_core::tx::{ScriptPublicKey, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput};
    use calico_txscript_errors::TxScriptError;

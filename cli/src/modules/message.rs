use calico_addresses::Version;
use calico_bip32::secp256k1::XOnlyPublicKey;
use calico_wallet_core::{
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<CalicoCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, calico_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<CalicoCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (
                    "verify <calico_address> <signature>",
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<CalicoCli>, calico_address: &str, message: &str) -> Result<()> {
        let calico_address = Address::try_from(calico_address)?;
        if calico_address.version != Version::PubKey {
        ...
        let privkey = self.get_address_private_key(&ctx, calico_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<CalicoCli>, calico_address: &str, signature: &str, message: &str) -> Result<()> {
        let calico_address = Address::try_from(calico_address)?;
        if calico_address.version != Version::PubKey {
            return Err(Error::custom("Unsupported address for message verification. Only PubKey addresses are supported."));
        }

        let pubkey = XOnlyPublicKey::from_slice(&calico_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<CalicoCli>, calico_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&calico_address])?;

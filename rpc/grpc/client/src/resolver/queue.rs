use crate::{
    error::{Error, Result},
    resolver::{matcher::Matcher, CalicodResponseReceiver, CalicodResponseSender, Resolver},
};
...
struct Pending {
    ...
    sender: CalicodResponseSender,
}

impl Pending {
    fn new(op: CalicodPayloadOps, request: CalicodRequest, sender: CalicodResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &CalicodResponse, response_op: CalicodPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    fn register_request(&self, op: CalicodPayloadOps, request: &CalicodRequest) -> CalicodResponseReceiver {
        let (sender, receiver) = oneshot::channel::<Result<CalicodResponse>>();
        {
    ...
    fn handle_response(&self, response: CalicodResponse) {
        let response_op: CalicodPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");

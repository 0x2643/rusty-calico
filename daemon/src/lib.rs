...
pub use calicod::{Calicod, CalicodConfig, CalicodCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../calico-cpu-miner/target/debug",
    "../../calico-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub calicod: Option<Arc<dyn CalicodCtl + Send + Sync + 'static>>,
    // pub calicod_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { calicod: None, cpu_miner: None }
    }

    pub fn with_calicod(mut self, calicod: Arc<dyn CalicodCtl + Send + Sync + 'static>) -> Self {
        self.calicod = Some(calicod);
        self
    }
    ...
    pub fn calicod(&self) -> Arc<dyn CalicodCtl + Send + Sync + 'static> {
        self.calicod.as_ref().expect("accessing Daemons::calicod while calicod option is None").clone()
    }

    pub fn try_calicod(&self) -> Option<Arc<dyn CalicodCtl + Send + Sync + 'static>> {
        self.calicod.clone()
    }

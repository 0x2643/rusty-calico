//!
//! Calico [`Address`] implementation.
//!
//! In it's string form, the Calico [`Address`] is represented by a `bech32`-encoded
...
/// Address prefix identifying the network type this address belongs to (such as `calico`, `calicotest`, `calicosim`, `calicodev`).
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[borsh(use_discriminant = true)]
pub enum Prefix {
    #[serde(rename = "calico")]
    Mainnet,
    #[serde(rename = "calicotest")]
    Testnet,
    #[serde(rename = "calicosim")]
    Simnet,
    #[serde(rename = "calicodev")]
    Devnet,
    #[cfg(test)]
    A,
    #[cfg(test)]
...
impl Prefix {
    fn as_str(&self) -> &'static str {
        match self {
            Prefix::Mainnet => "calico",
            Prefix::Testnet => "calicotest",
            Prefix::Simnet => "calicosim",
            Prefix::Devnet => "calicodev",
            #[cfg(test)]
            Prefix::A => "a",
            #[cfg(test)]
            Prefix::B => "b",
...
impl TryFrom<&str> for Prefix {
    ...
    fn try_from(prefix: &str) -> Result<Self, Self::Error> {
        match prefix {
            "calico" => Ok(Prefix::Mainnet),
            "calicotest" => Ok(Prefix::Testnet),
            "calicosim" => Ok(Prefix::Simnet),
            "calicodev" => Ok(Prefix::Devnet),
            #[cfg(test)]
            "a" => Ok(Prefix::A),
            #[cfg(test)]
            "b" => Ok(Prefix::B),
...
///
///  Calico `Address` version (`PubKey`, `PubKey ECDSA`, `ScriptHash`)
///
/// @category Address
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[repr(u8)]
...
/// Calico [`Address`] struct that serializes to and from an address format string: `calico:qz0s...t8cv`.
///
/// @category Address
...
mod tests {
    ...
    fn cases() -> Vec<(Address, &'static str)> {
        ...
        vec![
            ...
            (Address::new(Prefix::Testnet, Version::PubKey, &[0u8; 32]),      "calicotest:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqptytccrk"),
            (Address::new(Prefix::Testnet, Version::PubKeyECDSA, &[0u8; 33]), "calicotest:qyqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqx4sqrfe5"),
            (Address::new(Prefix::Testnet, Version::PubKeyECDSA, b"\xba\x01\xfc\x5f\x4e\x9d\x98\x79\x59\x9c\x69\xa3\xda\xfd\xb8\x35\xa7\x25\x5e\x5f\x2e\x93\x4e\x93\x22\xec\xd3\xaf\x19\x0a\xb0\xf6\x0e"), "calicotest:qxaqrlzlf6wes72en3568khahq66wf27tuhfxn5nytkd8tcep2c0vrsgklux69f"),
            (Address::new(Prefix::Mainnet, Version::PubKey, &[0u8; 32]),      "calico:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq0awszqg7"),
            (Address::new(Prefix::Mainnet, Version::PubKey, b"\x5f\xff\x3c\x4d\xa1\x8f\x45\xad\xcd\xd4\x99\xe4\x46\x11\xe9\xff\xf1\x48\xba\x69\xdb\x3c\x4e\xa2\xdd\xd9\x55\xfc\x46\xa5\x95\x22"), "calico:qp0l70zd5x85ttwd6jv7g3s3a8llzj96d8dncn4zmhv4tlzx5k2jyev4z2654"),
        ]
        // cspell:enable
    }
    ...
    fn test_errors() {
        // cspell:disable
        let address_str: String = "calico:qqqqqqqqqqqqq1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        ...
        let address_str: String = format!("calico:qqqqqqqqqqqqq{invalid_char}qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e");
        let address: Result<Address, AddressError> = address_str.try_into();
        ...
        let address_str: String = "calico1:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::InvalidPrefix("calico1".into())), address);

        let address_str: String = "calicoqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::MissingPrefix), address);

        let address_str: String = "calico:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4l".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);

        let address_str: String = "calico:qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqkx9awp4e".to_string();
        let address: Result<Address, AddressError> = address_str.try_into();
        assert_eq!(Err(AddressError::BadChecksum), address);
        // cspell:enable
    }
    ...
    pub fn test_wasm_serde_constructor() {
        let str = "calico:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjxa3h2n6v";
    ...
    pub fn test_wasm_js_serde_object() {
        let expected = Address::constructor("calico:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjxa3h2n6v");
        ...
        obj.set("prefix", &JsValue::from_str("calico")).unwrap();

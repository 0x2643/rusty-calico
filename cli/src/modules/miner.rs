use crate::imports::*;
use calico_daemon::{locate_binaries, CpuMinerConfig};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "calico-cpu-miner").await {
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<CalicoCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<CalicoCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<CalicoCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = calico_daemon::locate_binaries(root.as_str(), "calico-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No calico-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select calico-cpu-miner binary", &binaries).await? {

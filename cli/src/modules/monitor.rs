...
impl Handler for Monitor {
    ...
    fn help(&self, _ctx: &Arc<dyn Context>) -> &'static str {
        "Monitors and displays the balance and synchronization status of the Calico wallet."
    }
...
impl Monitor {
    async fn main(self: Arc<Self>, ctx: &Arc<CalicoCli>, _argv: Vec<String>, _cmd: &str) -> Result<()> {
        let max_events = 16;
    ...
    async fn redraw(self: &Arc<Self>, ctx: &Arc<CalicoCli>, events: &Arc<Mutex<VecDeque<Box<Events>>>>) -> Result<()> {
        tprint!(ctx, "{}", ClearScreen);
        tprint!(ctx, "{}", Goto(1, 1));
        ...
        if !wallet.is_connected() {
            ...
            tprintln!(ctx, "{}", style("Calico node is currently syncing").magenta());
            tprintln!(ctx);
        }

//!
//! # WASM bindings for the [Calico p2p Node RPC client](CalicoRpcClient).
//!
//! This module provides a WASM interface for the Calico p2p Node RPC client
//! - [`RpcClient`].
//!
...
use calico_addresses::{Address, AddressOrStringArrayT};
use calico_consensus_client::UtxoEntryReference;
use calico_consensus_core::network::{NetworkType, NetworkTypeT};
use calico_notify::connection::ChannelType;
use calico_notify::events::EventType;
use calico_notify::listener;
use calico_notify::notification::Notification as NotificationT;
use calico_rpc_core::api::ctl;
pub use calico_rpc_core::wasm::message::*;
pub use calico_rpc_macros::{
    build_wrpc_wasm_bindgen_interface, build_wrpc_wasm_bindgen_subscriptions, declare_typescript_wasm_interface as declare,
};
use calico_wasm_core::events::{get_event_targets, Sink};
...
pub(crate) struct Inner {
    ...
    notification_channel: Channel<calico_rpc_core::Notification>,
}
...
///
///
/// Calico RPC client uses ([wRPC](https://github.com/workflow-rs/workflow-rs/tree/master/rpc))
/// interface to connect directly with Calico Node. wRPC supports
/// two types of encodings: `borsh` (binary, default) and `json`.
///
/// There are two ways to connect: Directly to any Calico Node or to a
...
/// ```
///
/// **Connecting to a Calico Node directly**
///
/// ```javascript
/// let rpc = new RpcClient({
...
/// to re-register event listeners, but your have to re-subscribe for Calico node
/// notifications:
///
/// ```typescript
/// rpc.addEventListener("connect", async (event) => {
...
impl RpcClient {
    pub fn new(config: Option<RpcConfig>) -> Result<RpcClient> {
        ...
        let client = Arc::new(
            CalicoRpcClient::new(encoding, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
...
impl RpcClient {
    ...
    /// Connect to the Calico RPC server. This function starts a background
    ...
    /// Disconnect from the Calico RPC server.
    ...
    /// however, you have to re-subscribe for Calico node notifications. As such, it is recommended
...
impl RpcClient {
    pub fn new_with_rpc_client(client: Arc<CalicoRpcClient>) -> RpcClient {
    ...
    pub fn client(&self) -> &Arc<CalicoRpcClient> {
        &self.inner.client
    }
                ...
                select_biased! {
                    ...
                    msg = notification_receiver.recv().fuse() => {
                        if let Ok(notification) = &msg {
                            match &notification {
                                calico_rpc_core::Notification::UtxosChanged(utxos_changed_notification) => {
...
impl RpcClient {
    ...
    pub fn parse_url(url: &str, encoding: Encoding, network: NetworkId) -> Result<String> {
        let url_ = CalicoRpcClient::parse_url(url.to_string(), encoding, network.into())?;
        Ok(url_)
    }
}
...
impl RpcClient {
    ...
    /// Difficulty Adjustment Algorithm (DAA) score changes in the Calico BlockDAG.
    ...
    /// Calico BlockDAG. The event notification will be scoped to the
    ...
    /// chain changes in the Calico BlockDAG.
...
build_wrpc_wasm_bindgen_subscriptions!([
    ...
    /// block is added to the Calico BlockDAG.
    BlockAdded,
    ...
    /// conflict occurs in the Calico BlockDAG.
    FinalityConflict,
    ...
    /// conflict in the Calico BlockDAG is resolved.
    ...
    /// score of the sink block changes in the Calico BlockDAG.
    ...
    /// UTXO set override for the pruning point changes in the Calico BlockDAG.
    ...
    /// template is generated for mining in the Calico BlockDAG.
    NewBlockTemplate,
]);
...
build_wrpc_wasm_bindgen_interface!(
    [
        ...
        /// Retrieves the current number of blocks in the Calico BlockDAG.
        ...
        /// structure of the Calico BlockDAG.
        ...
        GetBlockDagInfo,
        /// Returns the total current coin supply of Calico network.
        /// Returned information: Total coin supply.
        GetCoinSupply,
        /// Retrieves information about the peers connected to the Calico node.
        /// Returned information: Peer ID, IP address and port, connection
        /// status, protocol version.
        GetConnectedPeerInfo,
        /// Retrieves general information about the Calico node.
        /// Returned information: Version of the Calico node, protocol
        /// version, network identifier.
        /// This call is primarily used by gRPC clients.
        /// For wRPC clients, use {@link RpcClient.getServerInfo}.
        GetInfo,
        /// Provides a list of addresses of known peers in the Calico
        ...
        /// performance and status of the Calico node.
        ...
        /// the highest cumulative difficulty in the Calico BlockDAG.
        ...
        GetSinkBlueScore,
        /// Tests the connection and responsiveness of a Calico node.
        /// Returned information: None.
        Ping,
        /// Gracefully shuts down the Calico node.
        /// Returned information: None.
        Shutdown,
        /// Retrieves information about the Calico server.
        /// Returned information: Version of the Calico server, protocol
        /// version, network identifier.
        GetServerInfo,
        /// Obtains basic information about the synchronization status of the Calico node.
    ...
    [
        ...
        /// Adds a peer to the Calico node's list of known peers.
        /// Returned information: None.
        AddPeer,
        /// Bans a peer from connecting to the Calico node for a specified duration.
        /// Returned information: None.
        Ban,
        ...
        /// Retrieves the balance of a specific address in the Calico BlockDAG.
        /// Returned information: Balance of the address.
        GetBalanceByAddress,
        /// Retrieves balances for multiple addresses in the Calico BlockDAG.
        /// Returned information: Balances of the addresses.
        GetBalancesByAddresses,
        /// Retrieves a specific block from the Calico BlockDAG.
        /// Returned information: Block information.
        GetBlock,
        /// Retrieves multiple blocks from the Calico BlockDAG.
        ...
        /// Retrieves block headers from the Calico BlockDAG.
        /// Returned information: List of block headers.
        GetHeaders,
        /// Retrieves mempool entries from the Calico node's mempool.
        ...
        GetMempoolEntry,
        /// Retrieves information about a subnetwork in the Calico BlockDAG.
        ...
        /// Resolves a finality conflict in the Calico BlockDAG.
        /// Returned information: None.
        ResolveFinalityConflict,
        /// Submits a block to the Calico network.
        /// Returned information: None.
        SubmitBlock,
        /// Submits a transaction to the Calico network.
        /// Returned information: Submitted Transaction Id.
        SubmitTransaction,
        /// Submits an RBF transaction to the Calico network.

...
use calico_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use calico_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use calico_core::{info, trace};
use calico_database::prelude::{StoreError, StoreResult, DB};
use calico_hashes::Hash;
use calico_index_core::indexed_utxos::BalanceByScriptPublicKey;
use calico_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-calicod), for quicker resets, while still having a low memory footprint.

/// UtxoIndex indexes `CompactUtxoEntryCollections` by [`ScriptPublicKey`](calico_consensus_core::tx::ScriptPublicKey),
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<calico_consensus_core::tx::TransactionOutpoint>> {

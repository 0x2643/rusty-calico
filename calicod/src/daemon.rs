...
use calico_consensus_core::{
    config::ConfigBuilder,
    errors::config::{ConfigError, ConfigResult},
};
use calico_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use calico_core::{calicod_env::version, task::tick::TickService};
use calico_core::{core::Core, debug, info};
use calico_database::prelude::CachePolicy;
use calico_grpc_server::service::GrpcService;
use calico_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use calico_rpc_service::service::RpcCoreService;
use calico_txscript::caches::TxScriptCacheCounters;
use calico_utils::git;
use calico_utils::networking::ContextualNetAddress;
use calico_utils::sysinfo::SystemInfo;
use calico_utils_tower::counters::TowerConnectionCounters;

use calico_addressmanager::AddressManager;
use calico_consensus::{consensus::factory::Factory as ConsensusFactory, pipeline::ProcessingCounters};
use calico_consensus::{
...
use calico_p2p_flows::{flow_context::FlowContext, service::P2pService};

use calico_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use calico_utxoindex::{api::UtxoIndexProxy, UtxoIndex};
use calico_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the calicod process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the calicod
/// process. (Rusty Calico will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("rusty-calico");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".rusty-calico");
}
...
/// the application folder that contains calicod logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                calico_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, calico_utils::sync::semaphore_module_path()));
            } else {
                calico_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the Calicod command line to confirm all interactive questions)";
    ...
    let mut meta_db = calico_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = calico_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from calicod DB version
    ...
    {
        let msg =
            "Node database is from a different Calicod *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
        get_user_approval_or_exit(msg, args.yes);

        info!("Deleting databases from previous Calicod version");
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = calico_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", calico_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", calico_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", calico_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = calico_database::prelude::ConnBuilder::default()

...
use calico_consensus_core::api::counters::ProcessingCounters;
use calico_consensus_core::errors::block::RuleError;
...
use calico_index_core::indexed_utxos::BalanceByScriptPublicKey;
use calico_index_core::{
...
use calico_mining::feerate::FeeEstimateVerbose;
use calico_mining::model::tx_query::TransactionQuery;
use calico_mining::{manager::MiningManagerProxy, mempool::tx::Orphan};
use calico_notify::listener::ListenerLifespan;
use calico_notify::subscription::context::SubscriptionContext;
use calico_notify::subscription::{MutationPolicies, UtxosChangedMutationPolicy};
use calico_notify::{
...
use calico_p2p_flows::flow_context::FlowContext;
use calico_p2p_lib::common::ProtocolError;
use calico_perf_monitor::{counters::CountersSnapshot, Monitor as PerfMonitor};
use calico_rpc_core::{
    api::{
...
use calico_txscript::{extract_script_pub_key_address, pay_to_address_script};
use calico_utils::expiring_cache::ExpiringCache;
use calico_utils::sysinfo::SystemInfo;
use calico_utils::{channel::Channel, triggers::SingleTrigger};
use calico_utils_tower::counters::TowerConnectionCounters;
use calico_utxoindex::api::UtxoIndexProxy;
...
/// A service implementing the Rpc API at calico_rpc_core level.
///
...
pub struct RpcCoreService {
    ...
    fee_estimate_verbose_cache: ExpiringCache<calico_mining::errors::MiningManagerResult<GetFeeEstimateExperimentalResponse>>,
}
...
NOTE: This error usually indicates an RPC conversion error between the node and the miner. If you are on TN11 this is likely to reflect using a NON-SUPPORTED miner.",
    ...
    async fn get_block_template_call(
        ...
        if request.pay_address.prefix != self.config.prefix() {
            return Err(calico_addresses::AddressError::InvalidPrefix(request.pay_address.prefix.to_string()))?;
        }

        // Build block template
        let script_public_key = calico_txscript::pay_to_address_script(&request.pay_address);
    ...
    async fn estimate_network_hashes_per_second_call(
        ...
        const LEGACY_VIRTUAL: calico_hashes::Hash = calico_hashes::Hash::from_bytes([0xff; calico_hashes::HASH_SIZE]);

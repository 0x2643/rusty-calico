...
use calico_connectionmanager::ConnectionManager;
use calico_consensus_core::api::{BlockValidationFuture, BlockValidationFutures};
use calico_consensus_core::block::Block;
use calico_consensus_core::config::Config;
use calico_consensus_core::errors::block::RuleError;
use calico_consensus_core::tx::{Transaction, TransactionId};
use calico_consensus_notify::{
    notification::{Notification, PruningPointUtxoSetOverrideNotification},
    root::ConsensusNotificationRoot,
};
use calico_consensusmanager::{BlockProcessingBatch, ConsensusInstance, ConsensusManager, ConsensusProxy};
use calico_core::{
    calicod_env::{name, version},
    debug, info,
    task::tick::TickService,
};
use calico_core::{time::unix_now, warn};
use calico_hashes::Hash;
use calico_mining::mempool::tx::{Orphan, Priority};
use calico_mining::{manager::MiningManagerProxy, mempool::tx::RbfPolicy};
use calico_notify::notifier::Notify;
use calico_p2p_lib::{
    common::ProtocolError,
    convert::model::version::Version,
    make_message,
    pb::{calicod_message::Payload, InvRelayBlockMessage},
    CalicodHandshake, ConnectionInitializer, Hub, PeerKey, PeerProperties, Router,
};
use calico_utils::iter::IterExtensions;
...
impl FlowContext {
    ...
    ///
    /// _GO-CALICOD: OnNewBlock + broadcastTransactionsAfterBlockAdded_

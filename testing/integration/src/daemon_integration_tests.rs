...
use calico_consensus::params::SIMNET_PARAMS;
use calico_consensus_core::header::Header;
use calico_consensusmanager::ConsensusManager;
use calico_core::{task::runtime::AsyncRuntime, trace};
use calico_grpc_client::GrpcClient;
use calico_notify::scope::{BlockAddedScope, UtxosChangedScope, VirtualDaaScoreChangedScope};
use calico_rpc_core::{api::rpc::RpcApi, Notification, RpcTransactionId};
...
async fn daemon_sanity_test() {
    ...
    // let total_fd_limit =  calico_utils::fd_budget::get_limit() / 2 - 128;
    let total_fd_limit = 10;
    let mut calicod1 = Daemon::new_random(total_fd_limit);
    let rpc_client1 = calicod1.start().await;
    assert!(rpc_client1.handle_message_id() && rpc_client1.handle_stop_notify(), "the client failed to collect server features");

    let mut calicod2 = Daemon::new_random(total_fd_limit);
    let rpc_client2 = calicod2.start().await;
...
async fn daemon_mining_test() {
    ...
    let mut calicod1 = Daemon::new_random_with_args(args.clone(), total_fd_limit);
    let mut calicod2 = Daemon::new_random_with_args(args, total_fd_limit);
    let rpc_client1 = calicod1.start().await;
    let rpc_client2 = calicod2.start().await;

    rpc_client2.add_peer(format!("127.0.0.1:{}", calicod1.p2p_port).try_into().unwrap(), true).await.unwrap();
    ...
    for i in 0..10 {
        let template = rpc_client1
            .get_block_template(Address::new(calicod1.network.into(), calico_addresses::Version::PubKey, &[0; 32]), vec![])
            .await
            .unwrap();
    ...
    let vc = rpc_client2
        .get_virtual_chain_from_block(
            calico_consensus::params::SIMNET_GENESIS.hash, //
            true,
        )
        .await
        .unwrap();
...
/// `cargo test --release --package calico-testing-integration --lib -- daemon_integration_tests::daemon_utxos_propagation_test`
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn daemon_utxos_propagation_test() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("calico-testing-integration-heap.json").build();

    calico_core::log::try_init_logger(
        "INFO,calico_testing_integration=trace,calico_notify=debug,calico_rpc_core=debug,calico_grpc_client=debug",
    );
    ...
    let miner_address =
        Address::new(calicod1.network.into(), calico_addresses::Version::PubKey, &miner_pk.x_only_public_key().0.serialize());
    ...
    let user_address =
        Address::new(calicod1.network.into(), calico_addresses::Version::PubKey, &user_pk.x_only_public_key().0.serialize());

    // Some dummy non-monitored address
    let blank_address = Address::new(calicod1.network.into(), calico_addresses::Version::PubKey, &[0; 32]);
    ...
    // Check that acceptance data contains the expected coinbase tx ids
    let vc = rpc_client2.get_virtual_chain_from_block(calico_consensus::params::SIMNET_GENESIS.hash, true).await.unwrap();
    assert_eq!(vc.removed_chain_block_hashes.len(), 0);
    ...
    let mut clients = vec![ListeningClient::connect(&calicod2).await, ListeningClient::connect(&calicod1).await];
...
async fn daemon_cleaning_test() {
    init_allocator_with_default_settings();
    calico_core::log::try_init_logger(
        "info,calico_grpc_core=trace,calico_grpc_server=trace,calico_grpc_client=trace,calico_core=trace",
    );
    ...
    {
        let total_fd_limit = 10;
        let mut calicod1 = Daemon::new_random_with_args(args, total_fd_limit);
        let dyn_consensus_manager = calicod1.core.find(ConsensusManager::IDENT).unwrap();
        let dyn_async_runtime = calicod1.core.find(AsyncRuntime::IDENT).unwrap();
        ...
        core = Arc::downgrade(&calicod1.core);

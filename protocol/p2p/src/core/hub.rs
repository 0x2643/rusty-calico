use crate::{common::ProtocolError, pb::CalicodMessage, ConnectionInitializer, Peer, Router};
use calico_core::{debug, info, warn};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: CalicodMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: CalicodMessage) {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: CalicodMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<CalicodMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();

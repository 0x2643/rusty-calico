extern crate calico_consensus;
extern crate calico_core;
extern crate calico_hashes;

use std::sync::Arc;

use calico_alloc::init_allocator_with_default_settings;
use calico_core::{info, signals::Signals};
...
pub fn main() {
    #[cfg(feature = "heap")]
    let _profiler = dhat::Profiler::builder().file_name("calicod-heap.json").build();
    ...
    match fd_budget::try_set_fd_limit(DESIRED_DAEMON_SOFT_FD_LIMIT) {
        Ok(limit) => {
            if limit < MINIMUM_DAEMON_SOFT_FD_LIMIT {
                ...
                println!("The calicod node requires a setting of at least {DESIRED_DAEMON_SOFT_FD_LIMIT} to operate properly.");

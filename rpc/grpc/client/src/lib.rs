...
use calico_core::{debug, error, trace};
use calico_grpc_core::{
    channel::NotificationChannel,
    ops::CalicodPayloadOps,
    protowire::{calicod_request, rpc_client::RpcClient, CalicodRequest, CalicodResponse, GetInfoRequestMessage},
...
use calico_utils::{channel::Channel, triggers::DuplexTrigger};
use calico_utils_tower::{
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(CalicodPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type CalicodRequestSender = async_channel::Sender<CalicodRequest>;
type CalicodRequestReceiver = async_channel::Receiver<CalicodRequest>;
...
/// Data flow:
/// ```
/// //   CalicodRequest -> request_send -> stream -> CalicodResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: CalicodRequestSender,
    request_receiver: CalicodRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<CalicodResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual CalicodRequest to CalicodResponse stream
        let mut stream: Streaming<CalicodResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: CalicodPayloadOps, request: impl Into<CalicodRequest>) -> Result<CalicodResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: CalicodRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<CalicodResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = calicod_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = calicod_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }

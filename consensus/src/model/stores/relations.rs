use calico_consensus_core::BlockHashSet;
use calico_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use calico_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use calico_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use calico_database::prelude::{DirectWriter, MemoryWriter};
use calico_database::prelude::{ReadLock, StoreError};
use calico_database::prelude::{StoreResult, DB};
use calico_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; calico_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use calico_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(calico_database::prelude::ConnBuilder::default().with_files_limit(10));

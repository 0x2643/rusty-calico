// Basic example of a Calico wRPC client that connects to a node
...
// Calico RPC primitives
use calico_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use calico_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Calico RPC client instance using Borsh encoding.
        let client = Arc::new(CalicoRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);

//!
//! The main [`RpcApi`] trait that defines all RPC methods available in the Rusty Calico p2p node.
//!
...
use calico_notify::{listener::ListenerId, scope::Scope, subscription::Command};
...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known calicod addresses in the current network (mainnet, testnet, etc.)

...
use calico_consensus::processes::transaction_validator::transaction_validator_populated::{
...
use calico_consensus_core::hashing::sighash_type::SIG_HASH_ALL;
use calico_consensus_core::subnets::SubnetworkId;
use calico_consensus_core::tx::{MutableTransaction, Transaction, TransactionInput, TransactionOutpoint, UtxoEntry};
use calico_txscript::caches::Cache;
use calico_txscript::pay_to_address_script;
use calico_utils::iter::parallelism_in_power_steps;
...
fn mock_tx(inputs_count: usize, non_uniq_signatures: usize) -> (Transaction, Vec<UtxoEntry>) {
    ...
    let dummy_prev_out = TransactionOutpoint::new(calico_hashes::Hash::from_u64_word(1), 1);

...
use calico_consensus_core::{subnets::SubnetworkConversionError, tx::TransactionId};
use calico_utils::networking::IpAddress;
...
pub enum RpcError {
    ...
    #[error(transparent)]
    AddressError(#[from] calico_addresses::AddressError),

    #[error(transparent)]
    NetworkTypeError(#[from] calico_consensus_core::network::NetworkTypeError),

    #[error(transparent)]
    NetworkIdError(#[from] calico_consensus_core::network::NetworkIdError),

    #[error(transparent)]
    NotificationError(#[from] calico_notify::error::Error),

    #[error(transparent)]
    MiningManagerError(#[from] calico_mining_errors::manager::MiningManagerError),

    #[error(transparent)]
    ConsensusError(#[from] calico_consensus_core::errors::consensus::ConsensusError),

    #[error(transparent)]
    ScriptClassError(#[from] calico_txscript::script_class::Error),

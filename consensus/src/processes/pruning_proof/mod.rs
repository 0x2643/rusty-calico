...
use calico_core::{debug, info, trace};
use calico_database::prelude::{CachePolicy, ConnBuilder, StoreResultEmptyTuple, StoreResultExtensions};
use calico_hashes::Hash;
use calico_pow::calc_block_level;
use calico_utils::{binary_heap::BinaryHeapExtensions, vec::VecExtensions};
use thiserror::Error;
...
impl PruningProofManager {
    ...
    pub fn import_pruning_points(&self, pruning_points: &[Arc<Header>]) {
        for (i, header) in pruning_points.iter().enumerate() {
            ...
            let state = calico_pow::State::new(header);
    ...
    pub fn validate_pruning_point_proof(&self, proof: &PruningPointProof) -> PruningImportResult<()> {
        ...
        let (db_lifetime, db) = calico_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
        ...
        for anticone_block in anticone.iter().copied() {
            ...
            for hash in ghostdag_chain {
                if let Entry::Vacant(e) = ghostdag_blocks.entry(hash) {
                    ...
                    // We fill `ghostdag_blocks` only for calicod-go legacy reasons, but the real set we
                    // send is `daa_window_blocks` which represents the full trusted sub-DAG in the antifuture
                    // of the pruning point which calicod-rust nodes expect to get when synced with headers proof
                    if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                        e.insert(TrustedHeader {
                            header: self.headers_store.get_header(hash).unwrap(),

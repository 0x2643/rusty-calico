...
use calico_consensus::params::Params;
use calico_consensus_core::network::{NetworkId, NetworkType};
use calico_core::{info, task::tick::TickService, trace};
use calico_math::Uint256;
use calico_notify::scope::VirtualDaaScoreChangedScope;
use calico_rpc_core::api::rpc::RpcApi;
use calico_txscript::pay_to_address_script;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), calico_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}

/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    calico_core::log::try_init_logger(
        "INFO, calico_core::time=debug, calico_rpc_core=debug, calico_grpc_client=debug, calico_notify=info, calico_notify::address::tracker=debug, calico_notify::listener=debug, calico_notify::subscription::single=debug, calico_mining::monitor=debug, calico_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    calico_core::panic::configure_panic();
    ...
    // Fold-up
    //
    calico_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    calico_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    calico_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=22610 --p2p=22611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    calico_core::log::try_init_logger(
        "INFO, calico_core::core=trace, calico_core::time=debug, calico_rpc_core=debug, calico_grpc_client=debug, calico_notify=info, calico_notify::address::tracker=debug, calico_notify::listener=debug, calico_notify::subscription::single=debug, calico_mining::monitor=debug, calico_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    calico_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    let prealloc_address =
        Address::new(NetworkType::Simnet.into(), calico_addresses::Version::PubKey, &prealloc_pk.x_only_public_key().0.serialize());
...
/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package calico-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]

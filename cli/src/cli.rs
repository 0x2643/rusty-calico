...
use calico_daemon::{DaemonEvent, DaemonKind, Daemons};
use calico_wallet_core::account::Account;
use calico_wallet_core::rpc::DynRpcApi;
use calico_wallet_core::storage::{IdT, PrvKeyDataInfo};
use calico_wrpc_client::{CalicoRpcClient, Resolver};
...
impl From<&CalicoCli> for Arc<Terminal> {
    fn from(ctx: &CalicoCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<CalicoCli> for CalicoCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for CalicoCli {
...
impl CalicoCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                calico_core::log::init_logger(None, "info");
            } else {
                calico_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let calico_cli = Arc::new(CalicoCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(calico_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                calico_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<CalicoRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("Calico NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Calico NG - received metrics event {metrics:?}")
                                }
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Calico node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Calico node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Calico node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Calico node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let calicod = self.daemons().try_calicod();
            ...
            if let Some(calicod) = calicod.as_ref() {
                calicod.mute(false).await?;
                calicod.stop().await?;
            }
...
impl Cli for CalicoCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "calico" {
                prompt.push(title);
            }
...
impl cli::Context for CalicoCli {
...
pub async fn calico_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    CalicoCli::init();

    let options = Options::new(terminal_options, None);
    let cli = CalicoCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Calico Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));

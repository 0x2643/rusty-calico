...
(async()=>{
    ...
    async function log_transactions(accountId){
        ...
        transactionsResult.transactions.forEach((tx)=>{
            ...
            list.push({
                Id: tx.id,
                Type: tx.data.type,
                Value: sompiToCalicoString(value(tx)||0)
            });
    ...
    try {
                ...
                case "balance":
                    ...
                    Object.keys(balance).map(id=>{
                        list.push({
                            ...
                            Mature: sompiToCalicoString(data.balance.mature),
                            Pending: sompiToCalicoString(data.balance.pending),
                            Outgoing: sompiToCalicoString(data.balance.outgoing),
        ...
        // Send calico to address
        let sendResult = await wallet.accountsSend({
            walletSecret,
            accountId: firstAccount.accountId,
            priorityFeeSompi: calicoToSompi("0.001"),
            destination:[{
                address: firstAccount.changeAddress,
                amount: calicoToSompi("1.5")
            }]
        });
        console.log("sendResult", sendResult);

        // Transfer calico between accounts
        let transferResult = await wallet.accountsTransfer({
            ...
            transferAmountSompi: calicoToSompi("2.4"),
        });

...
pub(crate) async fn create(
    ...
    if let Err(err) = wallet.network_id() {
        tprintln!(ctx);
        tprintln!(ctx, "Before creating a wallet, you need to select a Calico network.");
    ...
    if wallet.exists(Some(&filename)).await? {
        ...
        let overwrite = term.ask(false, "Are you sure you want to overwrite it (type 'y' to approve)?: ").await?.trim().to_lowercase();
        if overwrite != "y" {
            return Ok(());
        }
    }
    ...
    if !import_with_mnemonic {
        ...
        tpara!(
            ...
            the Calico stored in it. Keep your mnemonic safe. Write it down and \

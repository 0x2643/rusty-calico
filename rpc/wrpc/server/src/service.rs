...
use calico_rpc_core::api::ops::RpcApiOps;
...
/// ### CalicoRpcHandler
///
/// [`CalicoRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct CalicoRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl CalicoRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        CalicoRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for CalicoRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "calico" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<CalicoRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(CalicoRpcHandler::new(tasks, *encoding, core_service, options.clone()));

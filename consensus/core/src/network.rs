...
impl NetworkType {
    pub fn default_rpc_port(&self) -> u16 {
        match self {
            NetworkType::Mainnet => 22110,
            NetworkType::Testnet => 22210,
            NetworkType::Simnet => 22510,
            NetworkType::Devnet => 22610,
        }
    }
...
pub enum NetworkIdError {
    #[error("Invalid network name prefix: {0}. The expected prefix is 'calico'.")]
...
///
/// NetworkId is a unique identifier for a Calico network instance.
...
impl NetworkId {
    ...
    pub fn default_p2p_port(&self) -> u16 {
        ...
        match self.network_type {
            NetworkType::Mainnet => 22111,
            NetworkType::Testnet => match self.suffix {
                Some(10) => 22211,
                Some(11) => 22311,
                None | Some(_) => 22411,
            },
            NetworkType::Simnet => 22511,
            NetworkType::Devnet => 22611,
        }
    }
    ...
    /// Returns a textual description of the network prefixed with `calico-`
    pub fn to_prefixed(&self) -> String {
        format!("calico-{}", self)
    }

    pub fn from_prefixed(prefixed: &str) -> Result<Self, NetworkIdError> {
        if let Some(stripped) = prefixed.strip_prefix("calico-") {

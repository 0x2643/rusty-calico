use calico_wallet_core::account::BIP32_ACCOUNT_KIND;
use calico_wallet_core::account::LEGACY_ACCOUNT_KIND;
use calico_wallet_core::account::MULTISIG_ACCOUNT_KIND;
        ...
        match action.as_str() {
            ...
            "import" => {
                if argv.is_empty() {
                    ...
                    ctx.term().help(
                        &[
                            ("account import legacy-data", "Import Calico Desktop keydata file or Calico Network Web Wallet: https://wallet.calico-network.org/ data on the same domain"),
                            (
                                "account import mnemonic bip32",
                                "Import Bip32 (12 or 24 word mnemonics used by calicowallet, calico-mobile etc.)",
                            ),
                            (
                                "account import mnemonic legacy",
                                "Import accounts 12 word mnemonic used by legacy applications (Calico Desktop and Calico Network Web Wallet: https://wallet.calico-network.org/)",
                            ),
                            (
                ...
                match import_kind.as_ref() {
                    "legacy-data" => {
                        ...
                        if exists_legacy_v0_keydata().await? {
                                    ...
                                    Some(Arc::new(move |processed: usize, _, balance, txid| {
                                        if let Some(txid) = txid {
                                            tprintln!(
                                                ...
                                                sompi_to_calico_string(balance),
                                                processed,
                                                txid
                                            );
                                        } else if processed > 0 {
                            ...
                            return Err("Calico Desktop keydata file not found".into());
                        }
                    }
                    "mnemonic" => {
                        if argv.is_empty() {
                            ...
                            tprintln!(ctx, "Please use 'legacy' for 12-word Calico Desktop and Calico Network Web Wallet: https://wallet.calico-network.org/ mnemonics\r\n");
                            return Ok(());
                        }
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<CalicoCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ...
                (Calico Desktop and Calico Network Web Wallet: https://wallet.calico-network.org/). Use 'account import' for additional help.",
                ),
            ...
            .derivation_scan(
                ...
                Some(Arc::new(move |processed: usize, _, balance, txid| {
                    if let Some(txid) = txid {
                        ...
                        tprintln!(ctx_, "Scanned {} derivations, found {} SPR", processed, sompi_to_calico_string(balance));
                    }
                })),
            )
            .await?;

...
use calico_wallet_core::error::Error as WalletError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WrpcError(#[from] calico_wrpc_client::error::Error),

    #[error(transparent)]
    RpcError(#[from] calico_rpc_core::RpcError),
    ...
    #[error(transparent)]
    Daemon(#[from] calico_daemon::error::Error),
    ...
    #[error(transparent)]
    Bip32(#[from] calico_bip32::Error),

    #[error("private key {0} already exists")]
    PrivateKeyAlreadyExists(String),

    #[error(transparent)]
    MetricsError(calico_metrics_core::error::Error),

    #[error(transparent)]
    CalicoWalletKeys(#[from] calico_wallet_keys::error::Error),

    #[error(transparent)]
    PssbLockScriptSigError(#[from] calico_wallet_psst::error::Error),

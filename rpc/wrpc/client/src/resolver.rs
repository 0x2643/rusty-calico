//!
//! Module implementing [`Resolver`] client for obtaining public Calico wRPC endpoints.
//!
...
///
/// # Resolver - a client for obtaining public Calico wRPC endpoints.
///
/// This client operates against [Calico Resolver](https://github.com/aspectron/calico-resolver) service
/// that provides load-balancing and failover capabilities for Calico wRPC endpoints. The default
/// configuration allows access to public Calico nodes, while custom configurations can be supplied
/// if you are running your own custom Calico node cluster.
///
#[derive(Debug, Clone)]
pub struct Resolver {
    inner: Arc<Inner>,
...
impl Resolver {
    ...
    fn make_url(&self, url: &str, encoding: Encoding, network_id: NetworkId) -> String {
        ...
        format!("{url}/v{CURRENT_VERSION}/calico/{network_id}/{tls}/wrpc/{encoding}")
    }
    ...
    /// Obtain a Calico p2p [`NodeDescriptor`] from the resolver based on the supplied [`Encoding`] and [`NetworkId`].
    ...
    /// Returns a Calico wRPC URL from the resolver based on the supplied [`Encoding`] and [`NetworkId`].

...
use calico_grpc_core::{
    protowire::{
        ...
        CalicodRequest, CalicodResponse,
    },
...
impl Rpc for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn Stream<Item = Result<CalicodResponse, tonic::Status>> + Send + Sync + 'static>>;

    /// Handle the new arriving client connection
    async fn message_stream(
        &self,
        request: Request<tonic::Streaming<CalicodRequest>>,

...
pub enum GrpcServerError {
    ...
    RpcApiError(#[from] calico_rpc_core::error::RpcError),
    ...
    ClosedHandler(CalicodPayloadOps),
...
impl From<GrpcServerError> for calico_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => calico_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for calico_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => calico_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => calico_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        calico_notify::error::Error::ChannelSendError.into()
    }
}

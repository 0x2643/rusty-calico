...
use calico_notify::{events::EventType, scope::Scope, subscription::Command};
use calico_rpc_core::RpcResult;
use std::{
...
impl ListeningClient {
    pub async fn connect(calicod: &Daemon) -> Self {
        let client = calicod.new_multi_listener_client().await;

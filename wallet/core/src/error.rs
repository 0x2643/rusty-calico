...
use calico_bip32::Error as BIP32Error;
use calico_consensus_core::sign::Error as CoreSignError;
use calico_rpc_core::RpcError as CalicoRpcError;
use calico_wrpc_client::error::Error as CalicoWorkflowRpcError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] calico_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    CalicoRpcClientResult(#[from] CalicoRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    CalicoWorkflowRpcError(#[from] CalicoWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] calico_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] calico_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] calico_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] calico_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] calico_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] calico_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] calico_txscript::MultisigCreateError),

    #[error(transparent)]
    TxScriptError(#[from] calico_txscript_errors::TxScriptError),
    ...
    #[error(transparent)]
    Metrics(#[from] calico_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Psst(#[from] calico_wallet_psst::error::Error),

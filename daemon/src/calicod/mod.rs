...
impl Default for CalicodConfig {
    fn default() -> Self {
        Self {
            mute: false,
            path: None,
...
impl TryFrom<CalicodConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: CalicodConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no calicod path is specified".to_string()));
        }
...
impl Calicod {
    pub fn new(args: CalicodConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: CalicodConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Calico node is already running.".to_string()));
            }
        }
        ...
        // let options = CalicodOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("Calicod binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}

#[async_trait]
pub trait CalicodCtl {
    async fn version(&self) -> Result<String>;
    async fn configure(&self, config: CalicodConfig) -> Result<()>;

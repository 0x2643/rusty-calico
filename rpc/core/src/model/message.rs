...
use calico_consensus_core::api::stats::BlockCount;
use calico_core::debug;
use calico_notify::subscription::{context::SubscriptionContext, single::UtxosChangedSubscription, Command};
...
pub struct GetBlockTemplateRequest {
    /// Which calico address should the coinbase block reward transaction pay into
...
pub struct GetBlockTemplateResponse {
    ...
    /// Whether calicod thinks that it's synced.
    /// Callers are discouraged (but not forbidden) from solving blocks when calicod is not synced.
    /// That is because when calicod isn't in sync with the rest of the network there's a high
...
//
// This call is only available when this calicod was started with `--utxoindex`
//

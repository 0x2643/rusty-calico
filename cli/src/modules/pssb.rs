...
use calico_wallet_core::account::pssb::finalize_psst_one_or_more_sig_and_redeem_script;
use calico_wallet_psst::{
        ...
        match action.as_str() {
            ...
            "script" => {
                ...
                match subcommand.as_str() {
                    "lock" => {
                        let amount_sompi = try_parse_required_nonzero_calico_as_sompi_u64(argv.first())?;
                        let outputs = PaymentOutputs::from((script_p2sh, amount_sompi));
                        let priority_fee_sompi = try_parse_optional_calico_as_sompi_i64(argv.get(1))?.unwrap_or(0);
                        let abortable = Abortable::default();
                    ...
                    "unlock" => {
                        ...
                        // Get locked UTXO set.
                        let spend_utxos: Vec<calico_rpc_core::RpcUtxosByAddressesEntry> =
                            ctx.wallet().rpc_api().get_utxos_by_addresses(vec![script_p2sh.clone()]).await?;
                        let priority_fee_sompi = try_parse_optional_calico_as_sompi_i64(argv.first())?.unwrap_or(0) as u64;
                        ...
                        tprintln!(
                            ...
                            sompi_to_calico(total_locked_sompi)
                        );
            ...
            "parse" => {
                ...
                let pssb = Self::parse_input_pssb(argv.first().unwrap().as_str())?;
                tprintln!(ctx, "{}", pssb.display_format(ctx.wallet().network_id()?, sompi_to_calico_string_with_suffix));

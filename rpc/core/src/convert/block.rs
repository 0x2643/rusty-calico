...
use calico_consensus_core::block::{Block, MutableBlock};
    ...
    fn try_from(item: RpcBlock) -> RpcResult<Self> {
        Ok(Self {
            ...
            transactions: Arc::new(
                item.transactions
                    .into_iter()
                    .map(calico_consensus_core::tx::Transaction::try_from)
                    .collect::<RpcResult<Vec<calico_consensus_core::tx::Transaction>>>()?,
            ),
        })
    }
}
